...
/* In this exercise, the order of transformation is reversed. The matrix
*  is trasnlated then rotated. This results in the rectangle to move to
*  a new position vector first and then rotating from a new position
*  vector. Therefore, the rectangle appears to be spinning around the
*  screen.
*/

void TransformationsExercise1()
{
	// Create vertices for rectangle with colour and texture vertices
	float vertices[] = {
		// Position			  // Colour			  // Texture
...
void TransformationsExercise2()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Initialise matrix 1
		glm::mat4 first_matrix = glm::mat4(1.0f);

		// Rotate and translate
		first_matrix = glm::translate(first_matrix, glm::vec3(0.5f, -0.5f, 0.0f));
		first_matrix = glm::rotate(first_matrix, (float)glfwGetTime(), glm::vec3(0.0, 0.0, 1.0));

		// Inform shader of matrix uniform
		shader.setMat4("transform", first_matrix);
		...
		// Initialise matrix 2
		glm::mat4 second_matrix = glm::mat4(1.0f);

		// Variable scaling
		float scaling_factor = (float)sin(glfwGetTime());

		// Scale and translate
		second_matrix = glm::translate(second_matrix, glm::vec3(-0.5f, 0.5f, 0.0f));
		second_matrix = glm::scale(second_matrix, glm::vec3(scaling_factor, scaling_factor, scaling_factor));

		// Inform shader of matrix uniform
		shader.setMat4("transform", second_matrix);
